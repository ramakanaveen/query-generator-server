...
class DatabaseConnector:
    ...
    def _modify_query_for_pagination(self, query: str, page: int, page_size: int) -> str:
        ...
        # In q: `(offset + page_size) sublist offset _ table` gives you a slice
    ...
    def _create_count_query_single_line(self, query: str) -> str:
        ...
        # Try to optimize for KDB - replace select with count
        # Pattern: select [columns] from table [where conditions]
        # Transform to: exec count i from table [where conditions]
    ...
    async def execute_paginated(
        ...
        Args:
            ...
            query_complexity: "SINGLE_LINE" or "MULTI_LINE" (defaults to MULTI_LINE for safety)
        ...
        try:
            ...
            query_hash = self._get_query_hash(query + query_complexity)  # Include complexity in hash
            ...
            logger.error(f"Paginated query execution error: {str(e)}", exc_info=True)
