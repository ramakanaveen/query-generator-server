...
LLM-based validation for SQL queries (Starburst, PostgreSQL, MySQL, etc.).
...
from app.services.query_generation.prompts.validator_prompts import SQL_VALIDATION_PROMPT
...
class SQLValidator:
    """LLM-based validator for SQL queries"""
    ...
    async def validate(
        ...
        """
        Validate a SQL query using LLM.

        Args:
            user_query: Original user request
            generated_query: Generated SQL query
            schema_summary: Formatted schema information
            database_type: Specific SQL database (starburst, postgres, mysql, etc.)

        Returns:
            ValidationResult object
        """
        try:
            logger.info(f"Validating SQL query for {database_type}")
            ...
            if result.valid:
                ...
                logger.warning(f"❌ SQL validation failed: {len(result.critical_errors)} errors")
...
@timeit
async def validate_sql_query(state):
    """
    Node function to validate SQL query from state.
    ...
    try:
        # Create validator
        validator = SQLValidator(state.llm)
        ...
        # Log to thinking
        if result.valid:
            state.thinking.append("✅ SQL validation passed")
        else:
            state.thinking.append(f"❌ SQL validation failed: {len(result.critical_errors)} errors")
        ...
        logger.error(f"Error in SQL validation node: {str(e)}", exc_info=True)
